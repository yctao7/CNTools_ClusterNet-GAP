        ...
        for _ in range(num_reps):
            for idx, i in enumerate(instances):
                if args.objective == 'modularity':
                    ...
                    Y = model(features[i], bin_adj_for_loss[i])
                    this_loss = loss_fn(Y, bin_adj_for_loss[i])
                loss += this_loss
                if update:
        ...
        for e in range(args.train_epochs):
            ...
            for t, i in enumerate(sklearn.utils.shuffle(train_instances)):
                if args.objective == 'modularity':
                    ...
                    loss = loss_fn(mu, r, embeds, dist, bin_adj_train[i], train_object[i], args)
                if args.objective == 'cut':
                    Y = model_cluster(features_train[i], bin_adj_train[i])
                    loss = loss_fn(Y, bin_adj_train[i])
                ...
                if e - min_e >= 3 or e == args.train_epochs - 1:
                    if args.objective == 'modularity':
                        ...
                        for i in train_instances:
                            mu, r, embeds, dist = min_model_cluster(features_train[i], adj_train[i], num_cluster_iter)
                        ...
                        torch.save(min_model_cluster.state_dict(), open(f'{args.output_dir}/first_model_{e}_{args.clustertemp}', 'wb'))
                        pickle.dump(min_loss_test, open(f'{args.output_dir}/results_distributional_{e}_{args.clustertemp}.pickle', 'wb'))
                    elif args.objective == 'cut':
                        Ys = []
                        for i in train_instances:
                            Y = min_model_cluster(features_train[i], bin_adj_train[i])
                            Ys.append(Y)
                        Ys = torch.cat(Ys)
                        labels = Ys.argmax(dim=1)
                        torch.save(Ys, open(f'{args.output_dir}/{args.input_adj_path.split("/")[-1]}first', 'wb'))
                        torch.save(labels, open(f'{args.output_dir}/{args.input_adj_path.split("/")[-1]}first_labels', 'wb'))
                        pickle.dump(min_loss_test, open(f'{args.output_dir}/{args.input_adj_path.split("/")[-1]}first_loss.pickle', 'wb'))
                    break
        # if args.objective == 'modularity':
