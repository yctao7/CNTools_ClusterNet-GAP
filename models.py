...
def cluster(data, k, temp, num_iter, init = None, cluster_temp=5):
    ...
    data = nn.functional.normalize(data) # torch.diag(1./torch.norm(data, p=2, dim=1)) @ data
...
#        dist = torch.cosine_similarity(data[:, None].expand(n, k, d).reshape((-1, d)), mu[None].expand(n, k, d).reshape((-1, d))).reshape((n, k))
        ...
        r = nn.functional.normalize(r, p=1, dim=0)
...
class GAP(nn.Module):
    def __init__(self, nfeat, nhid, nout, dropout):
        ...
        self.model = Sequential('x, edge_index', [
            (SAGEConv(nfeat, nhid), 'x, edge_index -> x'),
            nn.ReLU(),
            nn.Dropout(p=dropout),

            (SAGEConv(nhid, nout), 'x, edge_index -> x'),
            nn.ReLU(),
            nn.Dropout(p=dropout),
    ...
    def forward(self, x, adj):
        return self.model(x, adj.nonzero().T)
